...
def test_calls_add_up():
    ...
    assert ledger.models == ["us.anthropic.claude-sonnet-5"]
...
def test_the_models_that_answered_are_kept_in_order():
    ...
    with usage_scope() as ledger:
        record(
            model="us.anthropic.claude-sonnet-5", usage=TokenUsage(10, 10), cost_usd=0.1
        )
        record(model="llama3.2:3b", usage=TokenUsage(10, 10), cost_usd=0.0)

    assert ledger.models == ["us.anthropic.claude-sonnet-5", "llama3.2:3b"]
...
def test_recording_outside_a_scope_is_not_an_error():
    ...
    record(model="us.anthropic.claude-sonnet-5", usage=TokenUsage(1, 1), cost_usd=0.1)
...
async def test_two_runs_at_the_same_time_do_not_mix():
    ...
    first, second = await asyncio.gather(
        draft("us.anthropic.claude-sonnet-5", 100, 0.01),
        draft("us.amazon.nova-lite-v1:0", 7, 0.002),
    )

    assert first.input_tokens == 200
    assert first.models == ["us.anthropic.claude-sonnet-5"]
    assert second.input_tokens == 14
    assert second.models == ["us.amazon.nova-lite-v1:0"]
